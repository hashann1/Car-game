...
class GameView(context: Context, var gameTask: GameTask) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        time = time + 10 + speed
        val myCarWidth = viewWidth / 5
        val myCarHeight = myCarWidth + 10
        val myCarX = myCarPosition * viewWidth / 3 + viewWidth / 15 + 25
        val myCarY = viewHeight - 2 - myCarHeight

        // Draw the player's car (car1)
        val car1 = resources.getDrawable(R.drawable.aa, null)
        car1.setBounds(
            myCarX,
            myCarY,
            myCarX + myCarWidth - 50, // Adjusted width to fit within lane
            myCarY + myCarHeight
        )
        car1.draw(canvas)
        ...
        for (i in otherCars.indices) {
            try {
                ...
                val car2 = resources.getDrawable(R.drawable.a, null)
                car2.setBounds(
